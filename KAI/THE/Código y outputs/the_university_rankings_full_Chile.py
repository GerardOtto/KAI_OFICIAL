...
Filters only Chilean universities. Saves results in both CSV and JSON formats.
"""
...
def filter_chile_universities(data):
    """Filter only universities from Chile."""
    ...
    for university in data["data"]:
        if university.get('location') == 'Chile':
            chile_universities.append(university)
    
    print(f"  Chilean universities found: {len(chile_universities)}")
    
    if not chile_universities:
        return None
    
    return {"data": chile_universities}
...
def save_outputs(year, data, name, category: str = "chile"):
...
def process_year(year):
    """Fetch and save both tables for a single year, filtered for Chile."""
    print(f"\n=== YEAR {year} ===")
    ...
    if rankings_data:
        chile_rankings = filter_chile_universities(rankings_data)
        if chile_rankings:
            filtered_rankings = filter_data_for_db(chile_rankings, year, RANKINGS_FIELDS)
            save_outputs(year, filtered_rankings, "rankings", category="chile")
    time.sleep(1)
    ...
    if key_stats_data:
        chile_stats = filter_chile_universities(key_stats_data)
        if chile_stats:
            filtered_key_stats = filter_data_for_db(chile_stats, year, KEY_STATISTICS_FIELDS)
            save_outputs(year, filtered_key_stats, "key_statistics", category="chile")
    time.sleep(1)
...
def process_subject(year: int, subject_slug: str) -> None:
    """Fetch rankings and key statistics for a subject in a given year, filtered for Chile."""
    ...
    if rankings_data:
        ...
        if chile_rankings:
            filtered_rankings = filter_data_for_db(chile_rankings, year, RANKINGS_FIELDS)
            save_outputs(year, filtered_rankings, f"{subject_slug}_rankings", category="chile")
    time.sleep(1)
    ...
    if key_stats_data:
        ...
        if chile_stats:
            filtered_key_stats = filter_data_for_db(chile_stats, year, KEY_STATISTICS_FIELDS)
            save_outputs(year, filtered_key_stats, f"{subject_slug}_key_statistics", category="chile")
    time.sleep(1)
...
def run_interactive() -> None:
    ...
    print("\n✅ Processing complete.")
    print("• Only Chilean universities are included.")
    print(f"• Data saved in outputs/csv/chile/ and outputs/json/chile/")
